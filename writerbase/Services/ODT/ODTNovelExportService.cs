...
/// <summary>
/// Service class responsible for exporting projects to ODT format in novel style.
/// Creates OpenDocument files with project name as title, table of contents, and chapter headings.
/// </summary>
public class ODTNovelExportService
{
    /// <summary>
    /// Exports a project to ODT format using OpenDocument Format.
    /// Creates an ODT document with project name as title and chapters with bold headers.
    /// </summary>
    ...
    private string CreateODTContent(Project project)
    {
        ...
        // Project title (centered, Heading1 style)
        content.AppendLine($"      <text:p text:style-name=\"Heading1\">{XmlEscape(project.Title)}</text:p>");
        
        // Page break
        content.AppendLine("      <text:p text:style-name=\"PageBreak\"/>");
        
        // Table of Contents title
        content.AppendLine("      <text:p text:style-name=\"Heading1\">Table of Contents</text:p>");
        content.AppendLine("      <text:p/>");
        
        // TOC entries
        foreach (var chapter in orderedChapters)
        {
            content.AppendLine($"      <text:p text:style-name=\"Heading2\">Chapter {chapter.Order}: {XmlEscape(chapter.Title)}</text:p>");
        }
    ...
    private string CreateODTStyles()
    {
        ...
        // Heading1 style (Title) - Courier, Bold, 32pt, Centered
        styles.AppendLine("    <style:style style:name=\"Heading1\" style:family=\"paragraph\">");
        ...
        // Heading2 style (Chapter headers) - Courier, Bold, 28pt
        styles.AppendLine("    <style:style style:name=\"Heading2\" style:family=\"paragraph\">");
        styles.AppendLine("      <style:text-properties fo:font-family=\"Courier New\" fo:font-size=\"28pt\" fo:font-weight=\"bold\"/>");
        styles.AppendLine("    </style:style>");
